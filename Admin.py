
import mysql.connector
import Tables
mydb = mysql.connector.connect(host="localhost", user="root", password="8092278097", database="library")
mycursor = mydb.cursor()

#---------------------------------------------------------------------------------------------------------                 
def displayAdmin():
    print()
    print("Admin Records: \n")
    mycursor.execute("SELECT * FROM AdminRecord")
    records=mycursor.fetchall()
    row_no = 0
    for rows in records :
        print(f"******************************Admin {row_no+1}******************************")
        print("\t             AdminID: ", rows[0])
        print("\t            Password: ", rows[1])
        print()
    x=input("Press Enter to continue")
    return
#---------------------------------------------------------------------------------------------------------         
def insertAdmin():
    while True :
        data=()
        print()
        AdminID=input("Enter AdminID: ")
        mycursor.execute("SELECT AdminID FROM AdminRecord where AdminID={0}".format("\'"+AdminID+"\'"))
        records=mycursor.fetchall()
        if records:
            print(f"AdminID {AdminID} already Exist !.")
        else: 
            Password=input(" Enter Password to be set: ")
            data=(AdminID,Password)
            query="INSERT INTO AdminRecord VALUES (%s, %s)"
            mycursor.execute(query,data)
            mydb.commit()
            print(f"Admin {AdminID} Add Successfully.\n")
        print()
        ch=input("Do you wish to do add more Administrators?[Yes/No] : ")
        if ch=="no" or ch=="No" or ch=="NO":
            break
    return
#---------------------------------------------------------------------------------------------------------         
def deleteAdmin():
    while True:
        print()
        AdminID=input(" Enter AdminID whose details to be deleted : ")
        mycursor.execute("SELECT AdminID FROM AdminRecord where AdminID={0}".format("\'"+AdminID+"\'"))
        records=mycursor.fetchall() 
        if records: 
            mycursor.execute("DELETE from AdminRecord where AdminID={0}".format("\'"+AdminID+"\'"))
            mydb.commit()
            print(f"Admin {AdminID} Delete Successfully.\n")
        else:
            print(f"AdminID:{AdminID} is not valid.\nPlease enter valid AdminID.\n")
        ch=input("Do you wish to delete more Administrators?[Yes/No] : ")
        if ch=="no" or ch=="No" or ch=="NO":
            break
    return
#---------------------------------------------------------------------------------------------------------     
def searchAdmin():
    while True:
        print()
        Search=input(" Enter AdminID to be Searched: ")  
        mycursor.execute("SELECT * FROM AdminRecord where AdminID={0}".format("\'"+Search+"\'"))
        records=mycursor.fetchall()
        row_no = 0
        if records:
            for rows in records :
                row_no+1
                print("******************************","Searched Admin Record","******************************")
                print("\t             AdminID: ", rows[0])
                print("\t            Password: ", rows[1])
                print()
        else:
            print(f" Search Unsuccesfull \nAdminID: {Search} is wrong. Please enter valid AdminID\n")
            
        ch=input("Do you wish to Search more Administrators?[Yes/No] : ")
        if ch=="no" or ch=="No" or ch=="NO":
            break
    return
#--------------------------------------------------------------------------------------------------------- 
def updateAdmin():
    while True:
        print()
        data=()
        AdminID=input(" Enter Admin ID for whose details need to be updated : ")
        mycursor.execute("SELECT AdminID FROM AdminRecord where AdminID={0}".format("\'"+AdminID+"\'"))
        records = mycursor.fetchall()
        if records:
            Password=input(" Enter new Password : ")
            query="UPDATE AdminRecord SET Password = %s WHERE AdminID=%s"
            data=(Password,AdminID)
            mycursor.execute(query,data)
            mydb.commit()
            print(f"Admin {AdminID} his password is Update Successfully.\n")
        else:
            print(f"AdminID {AdminID} is not valid.\n")
        ch=input("Do you wish to Search more Administrators?[Yes/No] : ")
        if ch=="no" or ch=="No" or ch=="NO":
            break
    return
#--------------------------------------------------------------------------------------------------------- 